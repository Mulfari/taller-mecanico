...
function IconPencil() {
  return <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>;
}
...
function Modal({ vehicle, onClose }: { vehicle: Vehicle; onClose: () => void }) {
  ...
  const [form, setForm] = useState<VehicleForm>({
    ...
    color: vehicle.color ?? "",
    vin: vehicle.vin ?? "",
    mileage: vehicle.mileage && vehicle.mileage > 0 ? String(vehicle.mileage) : "",
    notes: vehicle.notes ?? "",
  });
...
export default function EditVehicleButton({ vehicle }: { vehicle: Vehicle }) {
  const [open, setOpen] = useState(false);
  return (
    <>
      <button
        onClick={() => setOpen(true)}
        className="inline-flex items-center gap-2 border border-white/10 hover:border-white/20 text-gray-400 hover:text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors"
        aria-label="Editar vehículo"
      >
        <IconPencil />
        Editar
      </button>
      {open && <Modal vehicle={vehicle} onClose={() => setOpen(false)} />}
    </>
  );
}
