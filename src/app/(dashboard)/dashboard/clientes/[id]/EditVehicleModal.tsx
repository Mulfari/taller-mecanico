...
function IconPencil() {
  return <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>;
}
...
function Modal({ vehicle, clientId, onClose }: { vehicle: Vehicle; clientId: string; onClose: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setServerError(null);
    try {
      await updateVehicle(vehicle.id, clientId, {
      ...
      <div className="relative bg-[#16213e] border border-white/10 rounded-2xl w-full max-w-lg shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1.5">Placa</label>
...
export default function EditVehicleButton({ vehicle, clientId }: { vehicle: Vehicle; clientId: string }) {
  const [open, setOpen] = useState(false);
  return (
    <>
      <button
        onClick={() => setOpen(true)}
        className="inline-flex items-center gap-1 text-gray-500 hover:text-gray-300 text-xs px-2 py-1 rounded-md hover:bg-white/5 transition-colors"
        aria-label="Editar vehículo"
      >
        <IconPencil /> Editar
      </button>
      {open && <Modal vehicle={vehicle} clientId={clientId} onClose={() => setOpen(false)} />}
    </>
  );
}
