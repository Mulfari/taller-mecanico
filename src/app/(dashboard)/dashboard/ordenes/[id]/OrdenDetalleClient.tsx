        ...
        <div className="bg-secondary border border-white/10 rounded-xl p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-gray-500 text-xs">
              {new Date(clientRating.createdAt).toLocaleDateString("es-MX", {
                day: "2-digit",
                month: "short",
                year: "numeric",
              })}
          ...
          <div className="flex items-center gap-3 mb-2">
            <div className="flex items-center gap-0.5">
              {[1, 2, 3, 4, 5].map((star) => (
                <svg
                  key={star}
                  className={`w-5 h-5 ${star <= clientRating.rating ? "text-yellow-400" : "text-gray-600"}`}
                  fill={star <= clientRating.rating ? "currentColor" : "none"}
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                >
            ...
            <span className="text-sm text-yellow-400 font-medium">
              {clientRating.rating === 1 ? "Malo" : clientRating.rating === 2 ? "Regular" : clientRating.rating === 3 ? "Bueno" : clientRating.rating === 4 ? "Muy bueno" : "Excelente"}
            </span>
            <span className="text-gray-600 text-xs">— {clientRating.clientName}</span>
          </div>
          {clientRating.comment && (
            <p className="text-gray-300 text-sm bg-white/[0.03] rounded-lg px-4 py-3 border border-white/5">
