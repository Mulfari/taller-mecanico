...
export default async function SeguimientoPage({
  ...
  const activeOrders = result?.orders.filter((o) => o.status !== "delivered") ?? [];
  const pastOrders = result?.orders.filter((o) => o.status === "delivered") ?? [];
            ...
            <div className="flex items-center gap-4 bg-[#16213e] border border-white/10 rounded-2xl px-6 py-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 text-sm text-gray-500 mt-0.5">
                  ...
                  {result.vehicle.color && <span>{result.vehicle.color}</span>}
                </div>
              </div>
              <div className="ml-auto text-right shrink-0">
                <p className="text-gray-500 text-xs">Órdenes</p>
                <p className="text-white font-bold text-lg">{result.orders.length}</p>
              </div>
            </div>
